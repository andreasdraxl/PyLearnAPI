...
import launchprotocol
launchprotocol.report(__file__)
...
def elements_with_geometry_find(parameter_binding, view_id, type_exemplar_in_model = True):
	...
	cat_filter = ElementMulticategoryFilter(CList[ElementId](catigories_id))
...
def elements_with_geometry_find(parameter_binding, view_id, type_exemplar_in_model=True):
    ...
    if ex_parameter:
        elements = [el for el in FilteredElementCollector(doc, view_id).WherePasses(cat_filter).WhereElementIsNotElementType()]
    else:
        ...
        if type_exemplar_in_model:
            elements_ex = [str(el.GetTypeId()) for el in FilteredElementCollector(doc, view_id).WherePasses(cat_filter).WhereElementIsNotElementType()]
...
def Rooms():
	...
	return collector
def Rooms_R1_Level(elements, param_RE_Geschoss, param_room_R1):
    for room in elements:
        if room is not None:
            param = room.GetParameters(param_RE_Geschoss)
            target_param = room.GetParameters(param_room_R1)
            for p in param:
...
with revit.Transaction("RE_Geschoss Einsetzen"):
	all_parameters = project_parameter_find()
	new_view_id = three_d_view_create()
	parameter_names = "RE_Geschoss"
	ifc_parameter_name = "IfcSpatialContainer"
	param_room_R1_Geschoss = "RE_R1_Geschoss_kurz"
	...
	rooms = Rooms()
	main(rooms, par_1[0])
	Rooms_R1_Level(rooms, parameter_names, param_room_R1_Geschoss)
